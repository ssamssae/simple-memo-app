...
> **T-260804-084 (2026-08-04)** — 유료 AI 요약 기능이 앱에서 제거되어(T-260804-062 / PR#114)
> 스토어 문구 전면에서 「AI 요약」을 걷어냈다. 아니키 R3 ack 2026-08-04 23:50.
> ⚠️ 아직 안 끝난 것 = **스크린샷 3번 컷(`03_ai_summary.png`)이 없어진 기능 화면이다.**
> 문구만 고치면 스토어 페이지에 그 사진이 그대로 남는다 — 재촬영·번호 재정렬은 볼칸 레그.
...
```text
빠르게 기록하고 자연스러운 말로 관련 메모를 찾는 다크 메모장
```

## Google Play 긴 설명

```text
메모요 - 빠르게 기록하고 말로 찾는 다크 메모장
...
```text
빠르게 기록하고 자연스러운 문장으로 관련 메모를 찾으세요. 프리미엄 기간에는 배너 광고 없이 집중할 수 있습니다.
```

## iOS 키워드

```text
메모,메모장,노트,다크모드,심플,즐겨찾기,빠른메모,메모정리,일기,할일,말로검색
```
...
- 표시 이름: 메모요 프리미엄
- 설명: 말로 검색과 프리미엄 기간 광고 숨김을 이용하는 월간 구독입니다.
- 결제 주기: 1개월
- 대한민국 기준 가격: ₩1,900
- 구매 위치: 설정 > 메모요 프리미엄
...
- 필수 컷: 메모 목록, 말로 검색, 프리미엄 구매·복원 화면
- ⚠️ **재촬영 대상 (T-260804-084 미완)**: 현재 저장된 3번 컷 `03_ai_summary.png` 은 ★없어진 기능
  화면이다(`fastlane/screenshots/ko/iPhone 6.9 Display/` · `store/screenshots/ko/` 양쪽).
  문구만 고치고 이 컷을 그대로 올리면 스토어 페이지는 여전히 없는 기능을 보여준다.
  캡처 파이프라인(`integration_test/screenshot_test.dart`)의 `06-ai-summary` 시나리오는
  T-260804-062 에서 제거돼 그 컷을 더는 생성하지 않는다 ⇒ 재생성 + 번호 재정렬이 필요하다.
