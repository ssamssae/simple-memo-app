...
void main() {
  ...
  /// favorites 가 0건이면 SliverReorderableList 인스턴스 1개 (normals)
  /// favorites + normals 둘 다 있으면 2개. index 0 = favorites, 1 = normals.
  SliverReorderableList reorderableAt(WidgetTester tester, int index) {
  ...
  testWidgets('토글로 즐겨찾기 추가 후 normal reorder 호출이 즐겨찾기 메모를 안 건드린다', (
    tester,
  ) async {
    ...
    final memos = [
      Memo(id: 'n-1', content: 'N1 첫째\n본문', createdAt: now, updatedAt: now),
      Memo(
        id: 'n-2',
        content: 'N2 둘째\n본문',
    ...
    // 이제 favorites = [N3], normals = [N1, N2] → SliverReorderableList 2개
  ...
  testWidgets('즐겨찾기 그룹 안 reorder 가 일반 그룹 영향 0', (tester) async {
