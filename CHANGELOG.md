# Changelog

## [Unreleased]

### Added
- 메모 이미지 첨부 1단계 — 사진첩·카메라·클립보드 붙여넣기로 메모당 최대 10장, 긴 변 1600px JPEG 로 축소해 앱 문서 폴더 `attachments/` 에 저장하고 메모엔 파일명만 기록(`images` 키, 구버전 JSON 호환). 편집 화면 본문 아래 썸네일 줄(탭=전체화면 뷰어·핀치 확대, 길게=삭제), 목록·검색·휴지통 행 썸네일, 공유 시 사진 동봉. 영구삭제(개별·휴지통 비우기·30일 자동)와 함께 파일 정리, 앱 시작 시 고아 파일 정리. 사진은 백업에 포함되지 않음(백업 화면 안내) — 2단계 T-260829-024. (T-260829-022)

## [1.0.17] - 2026-08-03

### Fixed
- 밝은 테마에서 흰 글씨가 배경에 묻혀 사라지던 3화면 수정 — 휴지통, 프리미엄 안내, 설정의 온디바이스 검색 모델 항목. 하드코딩된 흰색을 테마 색으로 교체. (PR#110, T-260803-036)

### Release note (스토어 제출용)
- 밝은 테마에서 휴지통 등 일부 화면의 글씨가 배경과 같은 색이라 보이지 않던 문제를 수정했습니다.

## [1.0.16] - 2026-07-20

### Fixed
- 편집 화면 헤더 — 밝은 테마에서 편집(새메모/메모수정) 화면 상단 버튼(뒤로·저장·실행취소·공유)이 밝은 헤더와 색이 겹쳐 보이지 않던 문제 수정. 헤더를 본문과 동일한 다크 톤으로 고정. (PR#100, T-260720-024)

### Release note (스토어 제출용)
- 편집 화면의 밝은 테마에서 상단 버튼이 잘 보이지 않던 문제를 수정했습니다.

## [1.0.12] - 2026-07-11

### Fixed
- 마우스 휠 스크롤 — 메모 이동(드래그) 중에도 휠로 리스트가 스크롤되도록 수정. 휠로 화면을 넘긴 뒤 드롭할 때 발생 가능하던 크래시 경로 제거. (PR#82)
- 마우스 휠 스크롤 — 긴 메모 편집 화면·리스트에서 첫 화면 너머로 휠 스크롤이 안 되던 문제 수정. (PR#76)
- 검색 발췌 공백 정렬 — 검색 결과 발췌의 공백 매칭 불일치 수정. (PR#75)
- 빈 리스트 편집 컨트롤 — 메모가 없을 때 편집 컨트롤이 노출되던 문제 수정. (PR#74)

### Release note (스토어 제출용)
- 메모 이동 중 마우스 휠 스크롤이 동작하지 않던 문제를 수정했습니다.
- 긴 메모에서 마우스 휠 스크롤이 첫 화면에서 멈추던 문제를 수정했습니다.
- 검색 결과 발췌 표시를 개선했습니다.

## [1.0.7] - 2026-05-29

### Added
- Drive 1버튼 백업 — overflow 메뉴 "Drive 백업" 항목 추가. Google Drive `Memoyo` 폴더 자동 생성, JSON 파일 업로드 후 최근 7개 회전 보관. 성공 시 "Memoyo 폴더 열기" 액션 SnackBar.
- 삭제 실행 취소 — 단일 스와이프 삭제와 편집 모드 벌크 삭제 직후 SnackBar 의 "실행 취소" 액션 노출. 탭하면 원래 인덱스에 메모 재삽입 후 즐겨찾기/일반 그룹 순서 복원.
- 메모 공유 — 메모 편집 화면 AppBar 에 공유 IconButton 추가. 시스템 share sheet 로 본문 전달.

### Fixed
- Google OAuth 설정 정정 — Android `App signing key` SHA-1 교체, Drive 권한 동의 시트가 정상적으로 노출되도록 `drive.file` scope 명시.
- Drive 백업 실패 상태 분리 — 네트워크 끊김 / 권한 거부 / 용량 부족 / 기타 오류를 각각 다른 메시지 SnackBar 로 분기.

### Notes
- Drive 백업/가져오기 1버튼 흐름은 1.0.5 의 시스템 share sheet 기반 수동 백업/복원 위에 누적된 편의 레이어입니다. 가져오기 측 1버튼 (Drive 폴더 직접 탐색) 은 별도 트랙.
- 휴지통 30일 (1.0.7 4번째 트랙) 은 다음 사이클로 분리. 본 릴리즈는 실행 취소 SnackBar 까지.

## [1.0.5] - 2026-05-18

### Added
- 메모 백업/복원 — AppBar overflow 메뉴에 "메모 내보내기" / "메모 가져오기" 추가. 시스템 share sheet 를 통해 메일·AirDrop·iCloud Drive·Google Drive 등 자유 선택.
- 가져오기 되돌리기 — 직전 1단계 undo. 가져오기 직후 SnackBar 또는 overflow 메뉴에서 누름.
- 편집 모드 전체 선택 — "전체선택" / "선택해제" 토글 버튼.

### Removed
- 자동 동기화 (Firebase Firestore + Auth) 계획 폐기. 무료앱 + CS 부담 0 정책으로 수동 백업/복원 방식 채택. 상세: docs/superpowers/specs/2026-05-18-memo-export-import-design.md
